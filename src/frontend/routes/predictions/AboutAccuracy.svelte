...
<InfoDialog
  title="About Prediction Accuracy"
  classes="about_accuracy"
  maxWidth="800px"
  onClose={close}
>
  <p>
    ...
    always apply to the cluster as a whole and not to individual caves or taxa. The tab
    also provides estimates of the overall accuracy of the clustering, spanning all
    clusters.
  </p>
  <p>
  ...
  <p>
    ...
    fitting a power curve to the points associated with a range of recent visits to the
    cave and then extending the curve to the next visit or person-visit. Given that the
    data only has records of visits for which specimens were collected, it's clear that
    this technique will often be unreliable. Accuracy measurements of this technique can
    help you decide the degree to which to trust these predictions, or perhaps the
    degree to which you can treat these predictions as measures of the numbers of
    additional species that remain in the caves, independently of whether they would be
    found on the next visit or person-visit.
  </p>
  <p>
    The accuracy of predictions is determined separately for each cluster. Within a
    cluster, caves are sorted by the number of additional species predicted for the next
    visit or person-visit, highest count first. The prediction of the top cave in this
  ...
  <p>
    ...
    visits are first docked from all caves. The predictive technique is then applied to the
  ...
  <p>
    ...
    the actual sort, due to being unable to make predictions using fewer data points.
    For this reason, the number of caves in each top N of the predicted sort are also
  ...
  <p>
    ...
    The result is a single fraction for each top N group of the cluster indicating the fraction
    of caves correctly predicted to occur within the top N group of caves, on average, across
    the cluster.
  </p>
  <p>
    ...
    data. The page reports these fractions as percentages. For example, a top 5 accuracy
    of 25% would mean that 25% of the caves shown in the top N group can actually be
    expected to be among the top N best yielding caves on the next visit or
    person-visit. Put another way, a top 5 accuracy of 25% would mean that 25% of the
    top N predictions can be expected to have been correctly placed among the top N,
  ...
  <p>
    ...
    the visit similarly to the ordering of taxa by frequency in the predicted sort.
  </p>
  <p>
    As with the predictions for number of additional species, accuracy is measured for
    each top N predicted taxa. For each value of N, the taxa common to both the top N of
    the predicted sort and the top N of the actual sort are counted, and this count is
  ...
  <p>
    ...
    together, weighted by the number of taxa contributing the fractions. The result is a
    single fraction for each top N group of the cluster indicating the fraction of taxa correctly
    predicted to occur within the top N group of taxa, on average, across the cluster.
  </p>
  <p>
  ...
  <p>
    ...
    detail. However, you may still want to examine the details if you're trying to
    optimize the accuracy of predictions for a particular cave or group of caves.
  </p>
  <p>
  ...
  <p>
    ...
    the number of caves having at least two visits, which allows the cave to have at
    least one historical prediction to contribute to this accuracy measure.
  </p>
...
<style>
  :global(.about_accuracy) {
    margin: 0 2.5rem 1rem 2.5rem;
    font-size: 0.95rem;
  }

  :global(.about_accuracy h3) {
    font-size: 1.2rem;
    font-weight: bold;
  }
</style>
